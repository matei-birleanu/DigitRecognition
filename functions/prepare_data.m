...
function [train_mat, train_val] = prepare_data(name, no_train_images)
  ...
  % Încarcă datele din tabelul primit ca argument.
  data = load(name);
  
  % Salvează în matricea train_mat primele no_train_images linii din tabelul de imagini de antrenament.
  train_mat = data.trainX(1:no_train_images, :);
  
  % Salvează în vectorul train_val primele no_train_images valori ale vectorului de etichete.
